...
function launch(params::Dict{String,Any})::Nothing
    ...
    ##===================
    minr = bhm*(
        1 + sqrt(1 + (bhs/bhm))*sqrt(1 - (bhs/bhm))
    ...
    lin_f = Initialize_Field(name="lin_f",spin=psi_spin,boost=psi_spin,falloff=1,Mvals=Mv,nx=nx,ny=ny)
    lin_p = Initialize_Field(name="lin_p",spin=psi_spin,boost=psi_spin,falloff=1,Mvals=Mv,nx=nx,ny=ny)
    ...
    scd_f = Initialize_Field(name="scd_f",spin=psi_spin,boost=psi_spin,falloff=1,Mvals=Mv,nx=nx,ny=ny)
    scd_p = Initialize_Field(name="scd_p",spin=psi_spin,boost=psi_spin,falloff=1,Mvals=Mv,nx=nx,ny=ny)
    ...
    bkgrd_np = BackgroundNP.NP_0{prec}(Rvals=Rv,Yvals=Yv,Cvals=Cv,Svals=Sv,bhm=bhm,bhs=bhs,cl=cl)
    ##=============
    ## Initial data
    ##=============
