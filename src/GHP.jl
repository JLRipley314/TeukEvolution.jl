    ...
    function GHP_ops(;
        ...
        for j = 1:ny
            ...
            for i = 1:nx
                ...
                pre_edth_DT[i, j] =
                    (1.0 / sqrt(2.0)) *
                    (1.0 / ((cl^2) - im * bhs * R * cy)) *
                    (-im * bhs * sy)

                pre_edth_raised[i, j] =
                    (1.0 / sqrt(2.0)) * (1.0 / ((cl^2) - im * bhs * R * cy))

                pre_edth[i, j] =
                    ((im * bhs * R * sy / sqrt(2.0)) / ((im * (cl^2) + bhs * R * cy)^2))

                ##=============
                ## divided by R
                ##=============
                pre_edth_prime_DT[i, j] =
                    (1.0 / sqrt(2.0)) * (1.0 / ((cl^2) + im * bhs * R * cy)) * im * bhs * sy

                pre_edth_prime_lowered[i, j] =
                    (1.0 / sqrt(2.0)) * (1.0 / ((cl^2) + im * bhs * R * cy))

                pre_edth_prime[i, j] =
                    ((im * bhs * R * sy / sqrt(2.0)) / ((cl^2 + im * bhs * R * cy)^2))
                ...
                ## NOT divided by R
                ##=================
                pre_thorn[i, j] = ((1.0 / ((cl^4) + ((bhs * R * cy)^2))) * R * (im * bhs))

                pre_thorn_prime_DT[i, j] = ((2.0 + (4.0 * bhm * R / (cl^2))))
