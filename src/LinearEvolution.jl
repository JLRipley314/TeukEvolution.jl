...
function set_psi4_f_k!(; k::Array{ComplexF64,2}, p::Array{ComplexF64,2})::Nothing
    nx, ny = size(k)
    for j = 1:ny
        for i = 1:nx
            k[i, j] = p[i, j]
...
function set_psi3_k!(;
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] = (
                -4.0 * R[i] * mu_0[i, j] * psi3[i, j] - R[i] * tau_0[i, j] * psi4[i, j] + psi4_edth[i, j]
            )
        end
    end
    return nothing
...
function set_lam_k!(;
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] = (-R[i] * (mu_0[i, j] + conj(mu_0[i, j])) * lam[i, j] - psi4[i, j])
        end
    end
    return nothing
end
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] = (
                ...
                2.0 * R[i] * tau_0[i, j] * psi3[i, j] + psi3_edth[i, j]
            )
        end
    end
    return nothing
...
function set_hmbmb_k!(;
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] =
                (R[i] * (mu_0[i, j] - conj(mu_0[i, j])) * hmbmb[i, j] - 2.0 * lam[i, j])
        end
    end
    return nothing
end
...
function set_hlmb_k!(;
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] = (
                -R[i] * conj(mu_0[i, j]) * hlmb[i, j] - 2.0 * pi_l[i, j] -
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] = (
                ...
                (R[i]^2) *
                mu_0[i, j] *
                (conj(pi_0[i, j]) + 2.0 * tau_0[i, j]) *
                hlmb[i, j] - 2.0 * pi_edth[i, j] -
                2.0 * R[i] * conj(pi_0[i, j]) * pi_l[i, j] - 2.0 * psi2[i, j] -
                ...
                (R[i]^2) *
                (
                    -3.0 * mu_0[i, j] * pi_0[i, j] + 2.0 * conj(mu_0[i, j]) * pi_0[i, j] -
...
function finish_linear_reconstruction_k!(;
    ...
    for j = 1:ny
        for i = 1:nx
            k[i, j] +=
                (-((R[i] / cl)^2) * DR[i, j] - (falloff * R[i] / (cl^2)) * f[i, j]) /
...
function set_linear_k!(;
    psi4_f,
    psi4_f_l::Array{ComplexF64,2},
    psi4_p,
    psi4_p_l::Array{ComplexF64,2},
    psi3_f,
    psi3_l::Array{ComplexF64,2},
    psi2_f,
    psi2_l::Array{ComplexF64,2},
    lam_f,
    lam_l::Array{ComplexF64,2},
    pi_f,
    pi_l::Array{ComplexF64,2},
    pi_nm_l::Array{ComplexF64,2},
    hmbmb_f,
    hmbmb_l::Array{ComplexF64,2},
    hmbmb_f_nm,
    hmbmb_nm_l::Array{ComplexF64,2},
    hlmb_f,
    hlmb_l::Array{ComplexF64,2},
    hlmb_f_nm,
    hlmb_nm_l::Array{ComplexF64,2},
    muhll_f,
    muhll_l::Array{ComplexF64,2},
    Evo,
    Op,
    NP,
    R::Vector{Float64},
    ...
    set_psi3_k!(
        ...
        psi3 = psi3_l,
        psi4 = psi4_f_l,
        psi4_edth = psi4_f_edth,
        mu_0 = mu_0,
        tau_0 = tau_0,
        R = R,
    )
    finish_linear_reconstruction_k!(
        k = psi3_k,
        f = psi3_l,
        DR = psi3_DR,
        R = R,
        falloff = psi3_f.falloff,
        bhm = bhm,
        cl = cl,
        dr = dr,
    )
    ...
    set_muhll_k!(
        ...
        psi2 = psi2_l,
        pi_nm = pi_nm_l,
        hmbmb_edth_nm = hmbmb_edth_nm,
        hmbmb_nm = hmbmb_nm_l,
        hlmb_edth_nm = hlmb_edth_nm,
        hlmb_nm = hlmb_nm_l,
        mu_0 = mu_0,
        tau_0 = tau_0,
        pi_0 = pi_0,
...
function Linear_evolution!(;
    ...
    psi4_f_pm_n = psi4_f_pm.n
    psi4_f_nm_n = psi4_f_nm.n
    psi4_f_pm_tmp = psi4_f_pm.tmp
    psi4_f_nm_tmp = psi4_f_nm.tmp
    psi4_f_pm_np1 = psi4_f_pm.np1
    psi4_f_nm_np1 = psi4_f_nm.np1
    psi4_f_pm_k = psi4_f_pm.k
    psi4_f_nm_k = psi4_f_nm.k

    psi4_p_pm_n = psi4_p_pm.n
    psi4_p_nm_n = psi4_p_nm.n
    psi4_p_pm_tmp = psi4_p_pm.tmp
    psi4_p_nm_tmp = psi4_p_nm.tmp
    psi4_p_pm_np1 = psi4_p_pm.np1
    psi4_p_nm_np1 = psi4_p_nm.np1
    psi4_p_pm_k = psi4_p_pm.k
    psi4_p_nm_k = psi4_p_nm.k

    psi3_pm_n = psi3_pm.n
    psi3_nm_n = psi3_nm.n
    psi3_pm_tmp = psi3_pm.tmp
    psi3_nm_tmp = psi3_nm.tmp
    psi3_pm_np1 = psi3_pm.np1
    psi3_nm_np1 = psi3_nm.np1
    psi3_pm_k = psi3_pm.k
    psi3_nm_k = psi3_nm.k

    psi2_pm_n = psi2_pm.n
    psi2_nm_n = psi2_nm.n
    psi2_pm_tmp = psi2_pm.tmp
    psi2_nm_tmp = psi2_nm.tmp
    psi2_pm_np1 = psi2_pm.np1
    psi2_nm_np1 = psi2_nm.np1
    psi2_pm_k = psi2_pm.k
    psi2_nm_k = psi2_nm.k

    lam_pm_n = lam_pm.n
    lam_nm_n = lam_nm.n
    lam_pm_tmp = lam_pm.tmp
    lam_nm_tmp = lam_nm.tmp
    lam_pm_np1 = lam_pm.np1
    lam_nm_np1 = lam_nm.np1
    lam_pm_k = lam_pm.k
    lam_nm_k = lam_nm.k
    ...
    hmbmb_pm_n = hmbmb_pm.n
    hmbmb_nm_n = hmbmb_nm.n
    hmbmb_pm_tmp = hmbmb_pm.tmp
    hmbmb_nm_tmp = hmbmb_nm.tmp
    hmbmb_pm_np1 = hmbmb_pm.np1
    hmbmb_nm_np1 = hmbmb_nm.np1
    hmbmb_pm_k = hmbmb_pm.k
    hmbmb_nm_k = hmbmb_nm.k

    hlmb_pm_n = hlmb_pm.n
    hlmb_nm_n = hlmb_nm.n
    hlmb_pm_tmp = hlmb_pm.tmp
    hlmb_nm_tmp = hlmb_nm.tmp
    hlmb_pm_np1 = hlmb_pm.np1
    hlmb_nm_np1 = hlmb_nm.np1
    hlmb_pm_k = hlmb_pm.k
    hlmb_nm_k = hlmb_nm.k

    muhll_pm_n = muhll_pm.n
    muhll_nm_n = muhll_nm.n
    muhll_pm_tmp = muhll_pm.tmp
    muhll_nm_tmp = muhll_nm.tmp
    muhll_pm_np1 = muhll_pm.np1
    muhll_nm_np1 = muhll_nm.np1
    muhll_pm_k = muhll_pm.k
    muhll_nm_k = muhll_nm.k
    ...
    set_linear_k!(
        psi4_f = psi4_f_pm,
        psi4_f_l = psi4_f_pm_n,
        psi4_p = psi4_p_pm,
        psi4_p_l = psi4_p_pm_n,
        psi3_f = psi3_pm,
        psi3_l = psi3_pm_n,
        psi2_f = psi2_pm,
        psi2_l = psi2_pm_n,
    ...
    set_linear_k!(
        psi4_f = psi4_f_nm,
        psi4_f_l = psi4_f_nm_n,
        psi4_p = psi4_p_nm,
        psi4_p_l = psi4_p_nm_n,
        psi3_f = psi3_nm,
        psi3_l = psi3_nm_n,
        psi2_f = psi2_nm,
        psi2_l = psi2_nm_n,
    ...
    set_linear_k!(
        psi4_f = psi4_f_pm,
        psi4_f_l = psi4_f_pm_tmp,
        psi4_p = psi4_p_pm,
        psi4_p_l = psi4_p_pm_tmp,
        psi3_f = psi3_pm,
        psi3_l = psi3_pm_tmp,
        psi2_f = psi2_pm,
        psi2_l = psi2_pm_tmp,
        ...
        pi_nm_l = pi_nm_tmp,
        hmbmb_f = hmbmb_pm,
        hmbmb_l = hmbmb_pm_tmp,
        hmbmb_f_nm = hmbmb_nm,
        hmbmb_nm_l = hmbmb_nm_tmp,
        hlmb_f = hlmb_pm,
        hlmb_l = hlmb_pm_tmp,
        hlmb_f_nm = hlmb_nm,
        hlmb_nm_l = hlmb_nm_tmp,
        muhll_f = muhll_pm,
        muhll_l = muhll_pm_tmp,
        Evo = Evo_pm,
        Op = Op_pm,
        NP = NP,
        R = R,
    ...
    set_linear_k!(
        psi4_f = psi4_f_nm,
        psi4_f_l = psi4_f_nm_tmp,
        psi4_p = psi4_p_nm,
        psi4_p_l = psi4_p_nm_tmp,
        psi3_f = psi3_nm,
        psi3_l = psi3_nm_tmp,
        psi2_f = psi2_nm,
        psi2_l = psi2_nm_tmp,
        ...
        pi_nm_l = pi_pm_tmp,
        hmbmb_f = hmbmb_nm,
        hmbmb_l = hmbmb_nm_tmp,
        hmbmb_f_nm = hmbmb_pm,
        hmbmb_nm_l = hmbmb_pm_tmp,
        hlmb_f = hlmb_nm,
        hlmb_l = hlmb_nm_tmp,
        hlmb_f_nm = hlmb_pm,
        hlmb_nm_l = hlmb_pm_tmp,
        muhll_f = muhll_nm,
        muhll_l = muhll_nm_tmp,
        Evo = Evo_pm,
        Op = Op_nm,
        NP = NP,
        R = R,
    ...
    set_linear_k!(
        psi4_f = psi4_f_pm,
        psi4_f_l = psi4_f_pm_np1,
        psi4_p = psi4_p_pm,
        psi4_p_l = psi4_p_pm_np1,
        psi3_f = psi3_pm,
        psi3_l = psi3_pm_np1,
        psi2_f = psi2_pm,
        psi2_l = psi2_pm_np1,
        ...
        pi_nm_l = pi_nm_np1,
        hmbmb_f = hmbmb_pm,
        hmbmb_l = hmbmb_pm_np1,
        hmbmb_f_nm = hmbmb_nm,
        hmbmb_nm_l = hmbmb_nm_np1,
        hlmb_f = hlmb_pm,
        hlmb_l = hlmb_pm_np1,
        hlmb_f_nm = hlmb_nm,
        hlmb_nm_l = hlmb_nm_np1,
        muhll_f = muhll_pm,
        muhll_l = muhll_pm_np1,
        Evo = Evo_pm,
        Op = Op_pm,
        NP = NP,
        R = R,
    ...
    set_linear_k!(
        psi4_f = psi4_f_nm,
        psi4_f_l = psi4_f_nm_np1,
        psi4_p = psi4_p_nm,
        psi4_p_l = psi4_p_nm_np1,
        psi3_f = psi3_nm,
        psi3_l = psi3_nm_np1,
        psi2_f = psi2_nm,
        psi2_l = psi2_nm_np1,
        ...
        pi_nm_l = pi_pm_np1,
        hmbmb_f = hmbmb_nm,
        hmbmb_l = hmbmb_nm_np1,
        hmbmb_f_nm = hmbmb_pm,
        hmbmb_nm_l = hmbmb_pm_np1,
        hlmb_f = hlmb_nm,
        hlmb_l = hlmb_nm_np1,
        hlmb_f_nm = hlmb_pm,
        hlmb_nm_l = hlmb_pm_np1,
        muhll_f = muhll_nm,
        muhll_l = muhll_nm_np1,
        Evo = Evo_pm,
        Op = Op_nm,
        NP = NP,
        R = R,
...
function set_res_bianchi3!(;
    ...
    for j = 1:ny
        for i = 1:nx
            res_bianchi3[i, j] = (
                psi3_edth_prime[i, j] + 4.0 * pi_0[i, j] * psi3[i, j] - psi4_thorn[i, j] + rho_0[i, j] * psi4[i, j] -
    ...
    for j = 1:ny
        for i = 1:nx
            res_bianchi2[i, j] = (
                ...
                3.0 * psi2_0[i, j] * pi_f[i, j] - psi2_edth_prime[i, j] -
                3.0 * pi_0[i, j] * psi2[i, j] + psi3_thorn[i, j] -
